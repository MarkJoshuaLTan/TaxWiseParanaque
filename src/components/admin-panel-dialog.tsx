...
const AdminDashboard = forwardRef(({ settings: settingsProp, onSettingsChange, onSaveSuccess }: { settings: TaxSettings, onSettingsChange: (newSettings: TaxSettings) => void; onSaveSuccess: () => void; }, ref) => {
  ...
  const filteredLocations = editedSettings && selectedBarangay && editedSettings.taxData[selectedBarangay]
    ? Object.entries(editedSettings.taxData[selectedBarangay]).filter(([name]) =>
        selectedLocationName ? name === selectedLocationName : name.toLowerCase().includes(locationSearch.toLowerCase())
      )
    : [];
  ...
  return (
    <div className="w-full space-y-6 pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-3">
                  ...
                  <AutocompleteInput
                      ...
                      suggestions={editedSettings && selectedBarangay ? Object.keys(editedSettings.taxData[selectedBarangay]).map(name => ({ name, type: 'location' })) : []}
                      onInputChange={setLocationSearch}
                      onSelect={(item) => {
                          if (item) {
                              // Only set the selected location name, don't modify the search state
                              setSelectedLocationName(item.name);
                          } else {
                              // When null is passed (e.g., clearing the selection), reset both
                              setSelectedLocationName('');
                              setLocationSearch('');
                          }
                      }}
                      value={selectedLocationName ? { name: selectedLocationName, type: 'location' } : null}
                      onOpen={() => {
                          // Clear search when opening to show all locations
                          setLocationSearch("");
                      }}
                      disablePortal={true}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 w-full auto-rows-fr">
            {filteredLocations.length > 0 ? filteredLocations.map(([locationName, details]) => (
                <Card key={locationName} className="group glass-card border-black/5 dark:border-white/5 bg-white/60 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/[0.08] transition-all duration-500 rounded-[24px] border-l-2 border-l-primary/40 overflow-hidden shadow-xl w-full h-full flex flex-col">
                    ...
                    <CardContent className="px-8 pb-10 flex-1 flex flex-col justify-center">
